...
function updateCarouselImg() {
  ...
  const photoKey = baseKey + '_' + (currentCarouselIndex + 1);
  const altKey = baseKey + '_alt_' + (currentCarouselIndex + 1);
  let desc = window.i18nDict[photoKey];
  if (!desc) desc = window.i18nDict[baseKey] || '';
  document.getElementById('carousel-img').src = imgPath;
  document.getElementById('carousel-img').alt = window.i18nDict[altKey] || window.i18nDict[photoKey] || window.i18nDict[baseKey] || '';
