...
async function handleFormSubmit(event) {
  ...
  try {
    ...
    showNotification('success', 'Успешно!', 'Ваше сообщение отправлено успешно.');
    form.reset();
  } catch (error) {
    console.error('8. Error occurred:', error);
    console.error('Error message:', error.message);
    showNotification('error', 'Ошибка!', 'Произошла ошибка при отправке сообщения. Попробуйте еще раз.');
  }
}
