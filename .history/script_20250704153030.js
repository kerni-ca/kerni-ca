...
async function handleFormSubmit(event) {
  ...
  try {
    ...
    const successTitle = window.i18nDict['notification_success_title'] || 'Успешно!';
    const successMessage = window.i18nDict['notification_success_message'] || 'Ваше сообщение отправлено успешно.';
    showNotification('success', successTitle, successMessage);
    form.reset();
  } catch (error) {
    console.error('8. Error occurred:', error);
    console.error('Error message:', error.message);
    const errorTitle = window.i18nDict['notification_error_title'] || 'Ошибка!';
    const errorMessage = window.i18nDict['notification_error_message'] || 'Произошла ошибка при отправке сообщения. Попробуйте еще раз.';
